const STATUS_STYLES = {
  ...
  mixed: 'bg-green-400/5 border border-green-400/15 text-green-400/80',
}
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${STATUS_STYLES[status] || STATUS_STYLES.draft}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${
        ...
        status === 'returned' ? 'bg-amber-400' :
        status === 'mixed' ? 'bg-green-400/60' : 'bg-on-surface-variant'
      }`}></span>
